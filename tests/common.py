...
def mock_registry(hass, mock_entries=None):
    ...
    hass.data[entity_registry.DATA_REGISTRY] = hass.loop.create_task(_get_reg())
    return registry
...
def mock_device_registry(hass, mock_entries=None):
    ...
    hass.data[device_registry.DATA_REGISTRY] = hass.loop.create_task(_get_reg())
    return registry
...
def mock_restore_cache(hass, states):
    ...
    hass.data[key] = {state.entity_id: state for state in states}
    _LOGGER.debug("Restore cache: %s", hass.data[key])
    assert len(hass.data[key]) == len(states), "Duplicate entity_id? {}".format(states)
